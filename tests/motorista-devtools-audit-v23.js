...
(function(){
  ...
  function checkFinalizados(){
    const out = [];
    const jm = window.JM && window.JM.motorista;
    out.push(result('setCallsView existe', !!jm && typeof jm.setCallsView === 'function', jm && typeof jm.setCallsView));
    if (!jm || typeof jm.setCallsView !== 'function') return out;
    try {
      jm.setCallsView('finalizados');
      const text = document.body.innerText || '';
      out.push(result('aba Finalizados abre', /Finalizados|finalizado/i.test(text), text.slice(0, 240)));
      jm.setCallsView('ativos');
      out.push(result('volta para Em andamento', /Em andamento|andamento/i.test(document.body.innerText || ''), 'ok'));
    } catch (e) {
      out.push(result('aba Finalizados abre', false, e.message));
    }
    return out;
  }
  ...
  function run(){
    ...
    console.group('JMDriverAuditV23');
  ...
  window.JMDriverAuditV23 = { run };
  console.log('JMDriverAuditV23 carregado. Rode: JMDriverAuditV23.run()');
})();
